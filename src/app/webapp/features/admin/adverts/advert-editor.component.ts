...
@Component({ selector: 'app-advert-editor', standalone: true, imports: [ReactiveFormsModule, RouterLink, FileUploadComponent], templateUrl: './advert-editor.component.html', styleUrl: './advert-editor.component.scss' })
export class AdvertEditorComponent implements OnInit {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    startAt: ['', Validators.required], endAt: ['', Validators.required], sortPriority: [0, [Validators.required, Validators.min(0)]]
  });
  ...
  async save(): Promise<void> {
    ...
    try {
      if (this.mediaFiles.length) this.form.controls.mediaUrl.setValue(await this.service.uploadMedia(this.mediaFiles[0]));
      const input = this.toInput(); if (this.existing) await this.service.update(this.existing, input); else await this.service.create(input); await this.router.navigate(['/admin/adverts']);
    }
  ...
  private toInput(): AdvertInput {
    ...
    return { advertiserName:v.advertiserName.trim(), title:v.title.trim(), body:v.body.trim(), placement:v.placement, status:this.existing?.status ?? 'draft',
